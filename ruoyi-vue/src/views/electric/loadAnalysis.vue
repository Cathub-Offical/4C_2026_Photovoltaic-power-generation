...
import { getLoadAnalysis } from "@/api/analysis/load"
...
function getList() {
  ...
  getLoadAnalysis(params).then((res) => {
    ...
    // 从 itemList 直接计算 detailData，与图表保持同步
    if (itemList.length) {
      if (queryParams.value.timeType == "date") {
        const values = itemList.map((i) => parseFloat(i.value)).filter((v) => !isNaN(v))
        const maxVal = Math.max(...values)
        const minVal = Math.min(...values)
        const avg = values.reduce((a, b) => a + b, 0) / values.length
        const maxItem = itemList.find((i) => parseFloat(i.value) === maxVal)
        const minItem = itemList.find((i) => parseFloat(i.value) === minVal)
        detailData.value = {
          max: maxVal.toFixed(1),
          maxTime: maxItem ? maxItem.timeCode + ":00:00" : "--",
          min: minVal.toFixed(1),
          minTime: minItem ? minItem.timeCode + ":00:00" : "--",
          avg: avg.toFixed(1),
          rate: maxVal > 0 ? (avg / maxVal * 100).toFixed(1) + "%" : "--",
        }
      } else {
        const maxVals = itemList.map((i) => parseFloat(i.max)).filter((v) => !isNaN(v))
        const minVals = itemList.map((i) => parseFloat(i.min)).filter((v) => !isNaN(v))
        const avgVals = itemList.map((i) => parseFloat(i.avg)).filter((v) => !isNaN(v))
        const maxVal = Math.max(...maxVals)
        const minVal = Math.min(...minVals)
        const avg = avgVals.reduce((a, b) => a + b, 0) / avgVals.length
        const maxItem = itemList.find((i) => parseFloat(i.max) === maxVal)
        const minItem = itemList.find((i) => parseFloat(i.min) === minVal)
        detailData.value = {
          max: maxVal.toFixed(1),
          maxTime: maxItem ? maxItem.timeCode : "--",
          min: minVal.toFixed(1),
          minTime: minItem ? minItem.timeCode : "--",
          avg: avg.toFixed(1),
          rate: maxVal > 0 ? (avg / maxVal * 100).toFixed(1) + "%" : "--",
        }
      }
