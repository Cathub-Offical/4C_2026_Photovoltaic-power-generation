        ...
        <div class="total-alarm">
          {{
            `共计${queryParams.activeType === 0 ? "报警" : "未处理报警"}${
              queryParams.activeType === 0 ? (alarmInfo.todayAlarmCount ?? 0) : (alarmInfo.todayUnprocessed ?? 0)
            }条`
          }}
        </div>
...
function getAnalysis() {
  listHomeAlarm({}).then((res) => {
    ...
    pieData.value = [
      { name: "普通", value: res.data.todayLevel3 ?? 0 },
      { name: "严重", value: res.data.todayLevel2 ?? 0 },
      { name: "事故", value: res.data.todayLevel1 ?? 0 },
    ]
    drawEcharts()
  })
}
